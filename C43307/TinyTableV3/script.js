...
TINY.table=function(){
	...
	sorter.prototype.init=function(){
		...
		if(this.p.colddid){
			d=T$(this.p.colddid);
			var o=document.createElement('option'); o.value=-1; o.innerHTML='Filtrer dans la colonne'; d.appendChild(o)
		}
		for(i;i<t.w;i++){
			...
			if(this.p.avg){
				...
				for(i=0;i<t.w;i++){
					...
					if(this.p.avg.exists(i)){
						...
						s=isNaN(s)?'Je fais  ma demande':m=='$'?s=s.currency(t.c[i].decimals):s+m;
						j.innerHTML=s
					}else{j.innerHTML='&nbsp;'}
				}
			}
